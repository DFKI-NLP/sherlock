...
class EntityFishingPredictor(Predictor):
    ...
    def predict_document(self, document: Document) -> Document:
        ...
        if response.status_code == 200:
            data = response.json()
            if 'entities' in data:
                entities = EntityFishingPredictor._create_entities_from_mentions(document, data['entities'])
                document.ents.extend(entities)
        else:
            raise Exception(f"Entity Fishing API returned response code [{response.status_code}], "
                            f"message = [{response.reason}], body = [{response.text}]")
        return document

    @staticmethod
    def _create_entities_from_mentions(doc: Document, ef_entities: List[Dict[str, Any]]) -> List[Entity]:
        ...
        for ef_entity in ef_entities:
            if 'wikidataId' in ef_entity:
                ...
                wikipedia_ext_ref: str = ef_entity[
                    'wikipediaExternalRef'] if 'wikipediaExternalRef' in ef_entity else None
    ...
    @staticmethod
    def _find_matching_concept(doc: Document, mention_text: str, start: int, end: int) -> (int, Mention):
        if doc.ments:
            for (idx, m) in enumerate(doc.ments):
                char_start = doc.tokens[m.start].start
                char_end = doc.tokens[m.end - 1].end
                if char_start == start and char_end == end and doc.text[char_start:char_end] == mention_text:
                    return idx, m
        return -1, None