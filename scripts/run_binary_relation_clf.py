...
def train(args, dataset_reader, converter, model, tokenizer):
    ...
    train_dataset = load_and_cache_examples(args, dataset_reader, converter, tokenizer, split="train")
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0 and not args.tpu:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:  # Only evaluate when single GPU otherwise metrics may not average well
                        results, _ = evaluate(args, dataset_reader, converter, model, tokenizer, split="dev")
...
def main():
    ...
    parser.add_argument("--entity_handling", type=str, default="mask_entity", choices=["mark_entity", "mark_entity_append_ner", "mask_entity", "mask_entity_append_text"])
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result, _ = evaluate(args, dataset_reader, converter, model, tokenizer, split="dev", prefix=prefix)
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        model = model_class.from_pretrained(args.output_dir)
        model.to(args.device)
        result, predictions = evaluate(args, dataset_reader, converter, model, tokenizer, split="test")
